...
class SubscriptionManager {
    ...
    async checkSubscription(url) {
        try {
            ...
            if (this.enableProxyCheck && this.proxyUrl) {
                // 使用代理检测
                try {
                    content = await this.checkSubscriptionWithProxy(url);
                    console.log('代理检测成功');
                } catch (proxyError) {
            ...
            console.log(`订阅验证结果: ${isValid ? '有效' : '无效'}`);
            
            return isValid;
            
        } catch (error) {
            console.error(`订阅检测失败: ${url}`, error);
            return false;
        }
    }
        ...
        return new Promise((resolve, reject) => {
            chrome.runtime.sendMessage({
                ...
                if (chrome.runtime.lastError) {
                    reject(new Error(`运行时错误: ${chrome.runtime.lastError.message}`));
                } else if (response.success) {
                    resolve(response.content);
                } else {
                    reject(new Error(response.error || '代理检测失败'));
                }
            });
        });
    }
    ...
    validateSubscriptionContent(content) {
        ...
        const patterns = [
            ...
            /cipher:\s*\w+/,
            /protocol:\s*\w+/,
            /"type":\s*"(vmess|vless|trojan|ss|ssr)"/
        ];
